 ...
 *Модицицировал под Asus Zenfone 2 BORETS24, 2016
 *Modified for Asus Zenfone 2 BORETS24(4PDA/XDA)
 *
...
#define START_DELAY		HZ * 25
//#define MIN_INPUT_INTERVAL	150 * 1000L
#define DEFAULT_HISTORY_SIZE	14
#define DEFAULT_DOWN_LOCK_DUR	1000
//#define DEFAULT_BOOST_LOCK_DUR	3000 * 1000L
//#define DEFAULT_MIN_CPUS_ONLINE	2
#define DEFAULT_MAX_CPUS_ONLINE	NR_CPUS
#define DEFAULT_FAST_LANE_LOAD	45
#define DEFAULT_FAST_LANE_LOAD2	63
...
//	u64 boost_lock_dur;
	...
	.fast_lane_load2 = DEFAULT_FAST_LANE_LOAD2
};
...
static void msm_hotplug_work(struct work_struct *work)
{
	...
	if ((stats.cur_max_load >= hotplug.fast_lane_load) && (stats.cur_max_load < hotplug.fast_lane_load2)) {
		/* Enter the fast lane */
		online_cpu((hotplug.max_cpus_online - 1));
		goto reschedule;
	}else if (stats.cur_max_load >= hotplug.fast_lane_load2) {
		/* Enter the fast lane */
...
static ssize_t show_fast_lane_load2(struct device *dev,
				   struct device_attribute *msm_hotplug_attrs,
				   char *buf)
{
	return sprintf(buf, "%u\n", hotplug.fast_lane_load2);
}

static ssize_t store_fast_lane_load2(struct device *dev,
				    struct device_attribute *msm_hotplug_attrs,
				    const char *buf, size_t count)
{
	int ret;
...
static DEVICE_ATTR(fast_lane_load2, 644, show_fast_lane_load2,
		   store_fast_lane_load2);
